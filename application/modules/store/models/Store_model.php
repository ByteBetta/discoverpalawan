...
class Store_model extends CI_Model {
    ...
    public function deleteReview($id_review)
    {
        ...
        if($this->mUserBrowser->isLogged())
        {
            ...
            if(empty($errors) AND isset($data)  && $typeAuth == "admin" )
            {
    ...
    public function storeAccess($id){
        ...
        if(intval($statusUser) == 0)      {$data['status'] = 1;}
        else if(intval($statusUser) == 1) { $data['status'] = 0;}
        else { $errors["status"] = Translate::sprint(Messages::STATUS_NOT_FOUND); }
        ...
        foreach ($datas as $key => $data){
            ...
            if(TRUE){
                ...
                foreach ($votes->result() AS $value){
                    ...
                    if(!$new_data_results[$key]['votes']) { $new_data_results[$key]['votes'] = 0;}
    ...
    public function getStores($params=array(), $whereArray=array(), $callback = NULL){
        ...
        foreach ($new_stores_results as $key => $store){
            ...
            $new_stores_results[$key]['nbrOffers'] =  $this->db->where("store_id",$store['id_store'])->where("status",1)
            ...
            if(count($offer)>0){
                ...
                else if((isset($offer->value_type)  and $offer[0]->value_type == "price") and isset($offer[0]->offer_value)){
    ...
    public function updateStore($params=array()){
        ...
        {
            $data["detail"]= htmlentities($detail);
        }
    ...
    public function recentlyAdd(){
        ...
        if (GroupAccess::isGranted('store',DISPLAY_RECENTLY_ADDED))
        {
            $user_id = $this->mUserBrowser->getData("id_user");
            $this->db->where("user_id",$user_id);
        }
        ...
        $data = $this->db->get();
        return array("succces"=>1,"stores"=>$data->result(),"count"=>$c);
    }
