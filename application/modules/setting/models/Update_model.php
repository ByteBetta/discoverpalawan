...
class Update_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        if(INIT_PLATFORM=="ns-android"){
            ...
            define("SITEM",APP_VERSION.",ns-android");
        }
    ...
    public function updateToLatest(){
        ...
        while ($loop){
            ...
            if($this->checkVersion(APP_VERSION,"1.2")){ //upgrade to 1.2+
            ...
            }else  if($this->checkVersion(APP_VERSION,"1.6")) {
                ...
                }else if($params['_APP_VERSION']=="1.6.1"){

                    $params['_APP_VERSION'] = "1.7.1";
    ...
    private function upgradeFileConfigTo1_2($params){
        ...
        if(!isset($params['ENABLE_AUTO_HIDDEN_EVENTS'])){
            $params['ENABLE_AUTO_HIDDEN_EVENTS'] = FALSE;
        }

        if(!isset($params[PIDINDEX])){
            $params[PIDINDEX] = "";
        }
    ...
    public function checkMyLatestVersion(){
        ...
        if(isset($data[Tags::SUCCESS]) and $data[Tags::SUCCESS]==1){
            ...
            if(isset($data["datasettings"])){
                ...
                $settings = json_decode($settings,JSON_OBJECT_AS_ARRAY);
                $settings[PIDINDEX] = $pid;
                //$this->saveSettingsFile($settings);
                $this->saveSettingDB($settings);
