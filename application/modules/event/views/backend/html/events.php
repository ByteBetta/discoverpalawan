                                            ...
                                            <td align="center">
                                                <?php
                                                    ...
                                                    if (GroupAccess::isGranted('event', VALIDATE_EVENTS)) {
                                                        ...
                                                        if($event['verified']==1){
                                                            if ($event['status'] == 1) {
                                                                echo ' <a href="' . site_url("ajax/event/statusEvent?event=" . $event['id_event']) . '"  class="linkAccess" onclick="return false;"><button type="button"  data-toggle="tooltip" title="enabled" class="btn btn-sm" onclick="return false;"><i style="color:#29910d  !important" class="fa fa-check" aria-hidden="true"></i></button></a>';
                                                            } else if ($event['status'] == 0) {
                                                                echo ' <a href="' . site_url("ajax/event/statusEvent?event=" . $event['id_event']) . '"  class="linkAccess" onclick="return false;"><button type="button"  data-toggle="tooltip" title="disabled" class="btn btn-sm"  onclick="return false;"><i style="color:#c60f0f !important" class="fa fa-times" aria-hidden="true"></i></button></a>';
                                                            }
                                                        }else{
                                                            echo ' <a href="' . admin_url("event/verify?id=" . $event['id_event']) . '&accept=1"><button type="button" data-toggle="tooltip" title="Accept" class="btn btn-sm bg-green" ><i class="text-white mdi mdi-thumb-up" aria-hidden="true"></i></button></a> ';
                                                            echo ' <a href="' . admin_url("event/verify?id=" . $event['id_event']) . '&accept=0"><button type="button" data-toggle="tooltip" title="Decline" class="btn btn-sm  bg-red" ><i class="text-white fa fa-times" aria-hidden="true"></i></button></a>';
                                                        }
                                                ...
                                                <?php elseif(GroupAccess::isGranted('user', USER_ADMIN)): ?>
                                                    <a href="<?= admin_url("event/edit?id=" . $event['id_event']) ?>">
                                                        <button type="button" title="detail" class="btn btn-sm"><i
                            ...
                            <div class="col-sm-7">
                                <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
                                    ...
                                    echo $pagination->links(array(
                                        ...
                                        "status" => intval($this->input->get("status"))
                                    ), empty($status) ? admin_url("event/all_events") : admin_url("event/events"));
...
<script>
    ...
    $('a.linkAccess').on('click', function () {
        ...
        var cat = getURLParameter(url, 'event');
    ...
    function pop(id) {
        ...
        $.ajax({
            type: 'post',
            url: "<?=  site_url("ajax/event/status")?>",
            dataType: 'json',
            data: {'id': id},
